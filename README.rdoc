...
These assume you have already have an application in production and are adding
a slave configuration. Follow these closely -- order is significant.
...
* Configure settings using the <tt>mysql</tt> configuration hash -- you'll need
  to set the IPs or hostnames of your slaves and the interface MySQL should
  listen on for both master and slaves:
    :mysql:
...
* Edit <tt>database.yml</tt> to set your DB host to the address of the master
  interface, if it is currently set to localhost.
* Include the plugin and recipe(s) in your Moonshine manifest, *before* the
  <tt>default_stack</tt> recipe is called.
    ...
    plugin :mysql_slave
    recipe :mysql_slave
* <tt>cap deploy:setup HOSTFILTER=slave1.example.com,10.0.4.3</tt>
* <tt>cap deploy HOSTFILTER=slave1.example.com,10.0.4.3</tt>
* <tt>cap db:replication:setup</tt> -- This will momentarily lock tables!
* <tt>cap deploy HOSTFILTER=master-db.example.com</tt> -- This will trigger a
  MySQL restart if you're changing the bind address for the first time.
